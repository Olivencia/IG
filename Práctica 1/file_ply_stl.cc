...
//******************************************************************************
//  Función dibujar la figura completa
//******************************************************************************



int _file_ply::write(vector<float> &Vertices,vector<int> &Faces, bool n, bool a)
{
  glLineWidth(1.5);
  float p1_x,p2_x,p3_x, p1_y, p2_y, p3_y, p1_z, p2_z, p3_z;
    glBegin(GL_TRIANGLES);
    for(unsigned i=0; i<Faces.size(); i+=3){
        p1_x=Vertices[Faces[i]*3];
        p2_x=Vertices[Faces[i+1]*3];
        p3_x=Vertices[Faces[i+2]*3];
        p1_y=Vertices[Faces[i]*3+1];
        p2_y=Vertices[Faces[i+1]*3+1];
        p3_y=Vertices[Faces[i+2]*3+1];
        p1_z=Vertices[Faces[i]*3+2];
        p2_z=Vertices[Faces[i+1]*3+2];
        p3_z=Vertices[Faces[i+2]*3+2];

        if(!a){
            glColor3f(0,0,1);
            glVertex3fv((GLfloat *) &Vertices[Faces[i]*3]);
            glVertex3fv((GLfloat *) &Vertices[Faces[i+1]*3]);
            glVertex3fv((GLfloat *) &Vertices[Faces[i+2]*3]);
        }
        else
        {
            if(i%2==0){
...
//******************************************************************************
//  Función propuesta por el profesor como examen:
//  Mostrar la figura a partir del eye y>0 y divir en 4 cuadrantes de
//  diferente color
//******************************************************************************

int _file_ply::write_examen(vector<float> &Vertices,vector<int> &Faces, bool color, bool ajedrez)
{
  glLineWidth(1.5);
  float p1_x,p2_x,p3_x, p1_y, p2_y, p3_y, p1_z, p2_z, p3_z;
    if(!color){
  ...
  else
    {
      ...
      for(unsigned i=0; i<Faces.size(); i+=3){
        ...
        p3_z=Vertices[Faces[i+2]*3+2];
        if(p1_y>0 && p2_y>0 && p3_y>0){
          if((p1_x>0) && (p1_z>0) && (p2_x>0) && (p2_z>0) && (p3_x>0) && (p3_z>0)){
            glVertex3fv((GLfloat *) &Vertices[Faces[i]*3]);
            glVertex3fv((GLfloat *) &Vertices[Faces[i+1]*3]);
            glVertex3fv((GLfloat *) &Vertices[Faces[i+2]*3]);
          }
      ...
      for(unsigned i=0; i<Faces.size(); i+=3){
        ...
        p3_z=Vertices[Faces[i+2]*3+2];
        if(p1_y>0 && p2_y>0 && p3_y>0){
          if(!((((p1_x>0) && (p1_z>0) && (p2_x>0) && (p2_z>0) && (p3_x>0) && (p3_z>0))) ||  ((p1_x>0) && (p1_z<0) && (p2_x>0) && (p2_z<0) && (p3_x>0) && (p3_z<0)) || ((p1_x<0) && (p1_z>0) && (p2_x<0) && (p2_z>0) && (p3_x<0) && (p3_z>0))   ||  ((p1_x<0) && (p1_z<0) && (p2_x<0) && (p2_z<0) && (p3_x<0) && (p3_z<0)))){
            glVertex3fv((GLfloat *) &Vertices[Faces[i]*3]);
            glVertex3fv((GLfloat *) &Vertices[Faces[i+1]*3]);
            glVertex3fv((GLfloat *) &Vertices[Faces[i+2]*3]);
          }
